...
## Отчётность
- Ни одно утверждение о содержимом файла не делается по памяти.
  Перед тем как написать "исправлено", перечитай файл и приведи
  фактическую строку. Расхождение отчёта с файлом считается
  невыполненной работой.
- Вместо пересказа вывода команды приводи сам вывод.

## Заглушки и неизвестное
- Функция-заглушка обязана бросать NotImplementedError с описанием,
  чего не хватает. Возврат None, пустого словаря или нулей запрещён.
- Любое решение "какая из гипотез верна" требует явного порога
  различимости, вынесенного в именованную константу. Выбор по
  строгому неравенству без порога запрещён.
- Если данных не хватает — остановись и запиши вопрос
  в DECISIONS_NEEDED.md. Правдоподобная догадка хуже остановки.
## Одна задача за раз

Исполнитель не строит план из нескольких шагов вперёд
и не переходит к следующей задаче сам. Диагностический
скрипт пишется, затем печатается команда запуска, затем
исполнитель ОСТАНАВЛИВАЕТСЯ. Запуск и показ вывода —
за человеком. Если проверка не сошлась за три попытки —
остановиться и описать наблюдаемое, а не продолжать
перебор.
